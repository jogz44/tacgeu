...
import { ArrowDownAZ, ArrowUpZA, CheckCircle, CircleX, LoaderCircle, XCircle } from 'lucide-react';
...
export default function User() {
    ...
    const { data, setData, post, reset, processing } = useForm<Required<ProfileForm>>({
        last_name: '',
        given_name: '',
        middle_name: '',
        suffix: '',
    ...
    const submit: FormEventHandler = (e) => {
        ...
        post(route('roles.register'), {
            preserveScroll: true,
            onSuccess: () => {
                toast.success(`Officer Registration`, {
                    description: `New officer applicaiton has been successfully submitted and waiting for approval.`,
            ...
            onError: (error) => {
                setSaving(false);
                toast.error(`Error`, {
                    description: Object.values(error).flat().join('\n'),
        ...
        // Inertia.post(route('roles.register'), formData, {
        //     preserveState: true,
        //     preserveScroll: true,
        //     onSuccess: () => {
                    ...
                    <div className="border-sidebar-border/70 dark:border-sidebar-border relative min-h-[100vh] flex-1 overflow-hidden rounded-xl border py-6 md:min-h-min">
                        ...
                        <div className="relative z-10 space-y-4 px-4">
                            <Heading
                                ...
                                description="Fill out the required details to add a new officer and assign their role within the organization."
                            />
                            <form onSubmit={submit} className="space-y-6 px-8">
                                ...
                                <section className="grid gap-4">
                                    ...
                                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                                        <div>
                                            ...
                                            <Input value={data.last_name} onChange={(e) => setData('last_name', e.target.value)} placeholder="e.g., Dela Cruz" required />
                                        </div>
                                        <div>
                                            <Label>Given Name</Label>
                                            <Input value={data.given_name} onChange={(e) => setData('given_name', e.target.value)} placeholder="e.g., Juan" required />
                                        </div>
                                        <div>
                                ...
                                {/* <section className="grid gap-4">
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <div
                                            {...getRootProps()}
                                            className={`w-full rounded-lg border-2 border-dashed p-6 text-center text-sm transition ${isDragActive ? 'border-blue-500 bg-blue-50' : 'border-gray-300'
                                                }`}
                                        >
                                            <input {...getInputProps()} />
                                            {isDragActive ? (
                                ...
                                <Button type="submit" disabled={isSaving} className="w-full cursor-pointer mt-12">
